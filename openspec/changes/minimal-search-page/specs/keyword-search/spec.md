...
author, because the corpus is high-energy-physics literature in which collaboration
papers routinely carry thousands of author names and would otherwise dominate the
response. The total SHALL be reported alongside the sample, so that a caller can always
tell how many authors the paper really has and can recognise the list it received as a
sample rather than the whole. A caller needing every author has the INSPIRE link.

The size of a response SHALL therefore be bounded by the result cap and the author
sample, and SHALL NOT depend on how many authors the matched papers happen to have.
...
- **WHEN** one query matches papers with thousands of authors and another matches
  papers with a handful
