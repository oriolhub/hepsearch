...
- [x] An interface (protocol/ABC) declares `embed(texts: list[str]) -> list[list[float]]`
      and exposes its `dimension`
- [x] A local implementation wraps `sentence-transformers` with `all-MiniLM-L6-v2`
- [x] The provider is selected through settings, not imported directly by callers
- [x] The model is loaded lazily and only once per process, never per call
- [x] The model name and dimension are settings, documented in `.env.example`
- [x] `embed` accepts a batch and returns vectors in the same order as the input
- [x] Embedding an empty list returns an empty list without loading the model
- [x] A fake/deterministic provider exists for tests
- [x] Unit tests use the fake and **never download a model**
- [x] Exactly one test, marked `slow` and excluded from the default run, verifies
...
- [x] The default `uv run pytest` run stays fast and offline
- [x] The model cache directory is git-ignored

## Definition of done

- [x] Acceptance criteria met
- [x] `uv run pytest`, `uv run ruff check .` and `uv run ruff format --check .` pass
- [x] Committed as `HS-009: Add a pluggable embedding provider`
